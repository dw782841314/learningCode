...
import com.hdkj.modules.sys.dao.SysDeptDao;
...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptDao, SysDeptEntity> implements SysDeptService {
	...
	public List<SysDeptEntity> queryList(Map<String, Object> params){
		/*String sqlFiler = params.get("sql_filter").toString();
		//获取第一个（出现的位置
		int index = sqlFiler.indexOf("(");
		//获取第二个（出现的位置
		 index = sqlFiler.indexOf("(",index+1);
		 //获取第一个）出现的位置
		int indexof = sqlFiler.indexOf(")");
		String cord = sqlFiler.substring(index+1,indexof);*/
	...
	private void getDeptTreeList(List<String> subIdList, List<String> deptIdList){
		List<Long> subIdListl = new ArrayList<>();
		for (int i = 0 ; i<subIdList.size(); i++){
			System.out.println(subIdList.get(i));
			subIdListl.add(Long.valueOf(subIdList.get(i)));
		}
		for(Long deptId : subIdListl){
			List<String> list = queryDetpIdList(String.valueOf(deptId));
			if(list.size() > 0){
				getDeptTreeList(list, deptIdList);
			}

			deptIdList.add(String.valueOf(deptId));
		}
	}
}
